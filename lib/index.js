...
async function main () {
        ...
        for (const doc of docList) {
          ...
          try {
            if (docTitle === 'index.js') {
              ...
              dottie.set(output, `${pluginName}.stateResources.${stateResourceName}.description`, docContents.description, { force: true })
      ...
      for (const service of servicePaths) {
        ...
        for (const doc of docList) {
          ...
          try {
            if (docTitle === 'index.js') {
              docContents = require(doc)
              dottie.set(output, `${pluginName}.services.${serviceName}.description`, docContents.description, { force: true })
            } else if (docTitle === 'example.json') {
              docContents = JSON.parse(fs.readFileSync(doc))
  ...
  fs.appendFileSync('output.txt', JSON.stringify(output, null, 2))
}
