...
module.exports = class TymlyGatherer {
  ...
  async getPluginServices (pluginName, pluginPath) {
    // TODO: Loop over services in this dir and await this.addService(pluginName, servicePath)
    ...
    try {
      ...
      for (const serviceDir of servicePath) {
        debug(`Getting service data for: ${serviceDir}`)
        const serviceDocs = await glob(path.join(serviceDir, 'doc', '*'))
        for (const docPath of serviceDocs) {
          services.push(
            {
              'name': path.basename(serviceDir) || 'No doc was found',
              'contents': require(path.resolve(docPath)) || 'No doc was found'
            }
          )
        }
      }
      ...
      this.collected.plugins.push(
        {
          ...
          'services': await this.getPluginServices(pluginName, plugin),
          'stateResources': await this.getPluginStateResources()
        }
      )
    }
