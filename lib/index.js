...
module.exports = class TymlyGatherer {
  ...
  async getPluginServices (pluginName, pluginPath, messages) {
    ...
    if (servicePath.length !== 0) {
      for (const serviceDir of servicePath) {
        services.push(await
          this.getService(pluginName, serviceDir, messages)
        )
      }
    } else {
  ...
  async getPluginStateResources (pluginName, pluginPath, messages) {
    ...
    const stateResourcePath = await
      glob(path.join(pluginPath, 'lib', 'components', 'state-resources', '*'))
    if (stateResourcePath.length !== 0) {
      for (const stateResourceDir of stateResourcePath) {
        stateResources.push(await
          this.getStateResource(pluginName, stateResourceDir, messages)
        )
      }
    } else {
      ...
      this.collected.plugins.push(
        {
          ...
          'services':
            await
              this.getPluginServices(pluginName, plugin, this.collectionMessages),
          'stateResources':
            await
