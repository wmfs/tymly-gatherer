...
module.exports = class TymlyGatherer {
  constructor (options) {
    ...
    console.log('This is sourceDir ', this.sourceDir)
  }

  async listStateResourceSummary () {}
  ...
  async listPluginSummary () {
    ...
    for (const plugin of pluginDirPaths) {
      ...
      const stateResourcesPath = path.join(pluginsPath, 'lib', 'components', 'state-resources', '*')
      ...
      for (const stateResource of stateResources) {
        ...
        dottie.set(pluginSummary, `${pluginDetail.name}.stateResources.${stateResourceName}`, stateResourceDetail[stateResourceName])
      }
      for (const service of services) {
        const serviceDetail = await this.getServiceDetail(service)
        const serviceName = Object.keys(serviceDetail)[0]
        dottie.set(pluginSummary, `${pluginDetail.name}.services.${serviceName}`, serviceDetail[serviceName])
      }
    }
  ...
  async getPluginDetail (plugin) {
    // console.log('getting plugin detail for ', plugin)
  ...
  async getServiceDetail (service) {
    // console.log('getting service detail for: ', service)
    const serviceDetail = {}
