...
module.exports = class Index {
  ...
  async listPluginSummary () {
    ...
    for (const pluginPath of pluginDirPaths) {
      const pluginDetail = await this.getPluginDetail(pluginPath)
      const pluginName = path.basename(pluginDetail.name)
  ...
  async listServiceSummary (pluginsPath, target) {
    ...
    for (const pluginPath of pluginDirPaths) {
      const pluginName = path.basename(pluginPath)
      const servicesPath = path.join(pluginPath, 'lib', 'components', 'services', '*')
  ...
  async getStateResourceDetail (stateResource) {
    ...
    const stateResourceName = path.basename(stateResource)
