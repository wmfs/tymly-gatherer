...
describe('Test gatherer functions', function () {
  ...
  it('should run generateDocumentation function for a service with JSDocs', async () => {
    ...
    const indexContent = `C:\\development\\tymly\\packages\\tymly-gatherer\\test\\fixtures\\pretend-tymly\\plugins\\simpsons-plugin\\lib\\components\\services\\${serviceName}\\index.js`
    const outputDirPath = `C:\\development\\tymly\\packages\\tymly-gatherer\\test\\output\\${pluginName}`
  ...
  it('should run generateDocumentation function for a service without JSDocs and receive some warnings', async () => {
    ...
    const indexContent = `C:\\development\\tymly\\packages\\tymly-gatherer\\test\\fixtures\\pretend-tymly\\plugins\\${pluginName}\\lib\\components\\services\\moes-tavern\\index.js`
    const outputDirPath = `C:\\development\\tymly\\packages\\tymly-gatherer\\test\\output\\${pluginName}`
    const outputFile = `${serviceName}.md`
    await gatherer.generateDocumentation(
      indexContent,
    ...
    const doc = fs.readFileSync(path.join(outputDirPath, outputFile)).toString()
    expect(doc).to.eql(mdBoilerPlate)
    expect(messages[0]).to.eql('moes-tavern in simpsons-plugin does not contain any JSDocs')
  })
})
